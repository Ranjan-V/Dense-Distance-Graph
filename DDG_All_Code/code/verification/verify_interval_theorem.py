#!/usr/bin/env python3
"""Exact finite checks focused on Theorem 5.3 (one cyclic affected interval)."""
...
def run() -> dict:
    x=_combined_run()
    out={"status":"PASS" if x["interval_failures"]==0 else "FAIL",
         "arithmetic":x["arithmetic"],"affected_rows_checked":x["affected_rows_checked"],
         "interval_failures":x["interval_failures"]}
